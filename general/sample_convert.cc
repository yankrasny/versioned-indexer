...
{
    bool operator()(const linfo& l1, const linfo& l2)
    {
...
int main(int argc, char** argv)
{
    ...
    FILE* fsample = fopen("samples", "rb");
    int scount;
    fread(&scount, sizeof(unsigned), 1, fsample);
    unsigned* samples = new unsigned[scount];
    fread(samples, sizeof(unsigned), scount, fsample);
    fclose(fsample);
